...
const LIGHTING_CONFIG = {
  ...
  spot: { position: [-2, 6, 2] as [number, number, number], intensity: 80, angle: 0.6 },
  point: { position: [4, -1.5, 1] as [number, number, number], intensity: 10, distance: 10 },
};
...
const MATERIAL_CONFIG = {
  ...
  badgeInactive: { color: "#9c9c9c", roughness: 0.3, metalness: 0, opacity: 0.4 },
};
...
function ChemiseGLB({ selectedBadge }: { selectedBadge?: string | null }) {
  ...
  useFrame((state, delta) => {
    ...
    const target = isSelected ? ANIMATION_CONFIG.selected : ANIMATION_CONFIG.default;
    const lerpFactor = delta * ANIMATION_CONFIG.lerpSpeed;

    // Rotation
    meshRef.current.rotation.x = THREE.MathUtils.lerp(meshRef.current.rotation.x, target.rotation[0], lerpFactor);
    meshRef.current.rotation.y = THREE.MathUtils.lerp(meshRef.current.rotation.y, target.rotation[1], lerpFactor);
    meshRef.current.rotation.z = THREE.MathUtils.lerp(meshRef.current.rotation.z, target.rotation[2], lerpFactor);

    // Scale
    const newScale = THREE.MathUtils.lerp(meshRef.current.scale.x, target.scale, lerpFactor);
    meshRef.current.scale.set(newScale, newScale, newScale);

    // Position
    meshRef.current.position.x = THREE.MathUtils.lerp(meshRef.current.position.x, target.position[0], lerpFactor);
    meshRef.current.position.y = THREE.MathUtils.lerp(meshRef.current.position.y, target.position[1], lerpFactor);
  ...
  useLayoutEffect(() => {
    scene.traverse((obj: any) => {
      if (obj.type === "Group" && obj.name.startsWith("badge_")) {
        const isActive = selectedBadge && obj.name.toLowerCase() === `badge_${selectedBadge.toLowerCase()}`;
        const materialConfig = isActive ? MATERIAL_CONFIG.badgeActive : MATERIAL_CONFIG.badgeInactive;
  ...
  return (
    <div className="w-full h-full">
      <Canvas camera={{ position: [0, 0, 5], fov: 20 }} shadows={false}>
        ...
        <directionalLight position={directional.position} intensity={directional.intensity} color="#ffffff" />
        <spotLight position={spot.position} intensity={spot.intensity} angle={spot.angle} penumbra={0} color="#ffffff" />
        <pointLight position={point.position} intensity={point.intensity} color="#ffffff" distance={point.distance} decay={2} />
