...
* **Mise en place (Admin manuel) :**
    * Pas de Dashboard Admin. La création des données de test (etapes, Troupes, Objectifs, Assignations) se fait manuellement via `Prisma Studio` ou un script de `seed`.

* **Parcours `Chef` (minimal) :**
    * **Dashboard :** Peut voir ses etapes (une simple grille 2D suffit, la 3D est pour la V2).
    * **Objectifs :** Peut cliquer sur une etape et voir la liste des `Compétences` et `Réalisations`.
...
* **Parcours `Referent` (minimal) :**
    ...
    * **Sélection :** Peut utiliser le "Sélecteur de etape" dans sa sidebar.
    * **Dashboard (Onglet 1) :** Peut voir la liste des `Realisations` en attente (statut `SOUMISE`) pour le etape sélectionné.
